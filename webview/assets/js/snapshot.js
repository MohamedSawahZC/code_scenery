...
export const takeSnapshot = (filename) => {
  ...
  if (selectedValue === 'svg') {
    domtoimage.toSvg(snapshotContainerBackgroundNode, options)
    ...
    domtoimage.toBlob(snapshotContainerBackgroundNode, options)
      .then(function (blob) {
        resetStyles();
        window.saveAs(blob, `codescenery-${filename}.${selectedValue}`);
      });
  }else{
    domtoimage.toBlob(snapshotContainerBackgroundNode, options)
      .then(function (blob) {
        resetStyles();
        window.saveAs(blob, `codescenery-${filename}.jpg`);
      });
  }
};
